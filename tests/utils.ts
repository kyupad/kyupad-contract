...
export const generateWhiteListInvest = (size: number) => {
    ...
    for (let i = 0; i < size; i++) {
        ...
        arrayMerkleLeaf.push(
            walletAddress.toString() + '_' + randomNumber.toString()
        );

        arrayWallet.push(walletAddress)
        maxTotalTicket.push(randomNumber);
    }

    return {arrayMerkleLeaf, totalTicket, maxTotalTicket, arrayWallet};
};
